...
class MockApi {
    ...
    suspend fun productLocation(id: Long): List<Location> {
        delay(500L)
        return when (id) {
            11L -> listOf(Location.Moscow, Location.Voronezh)
            12L -> listOf(Location.Voronezh)
            13L -> listOf()
            22L -> listOf()
            else -> listOf(Location.Moscow, Location.Voronezh, Location.SaintPetersburg)
        }
    }
    ...
    companion object {
        val userInfo = UserInfo("Sergey", Location.Voronezh)

        val otherUserInfo = UserInfo("Admin", Location.Moscow)
